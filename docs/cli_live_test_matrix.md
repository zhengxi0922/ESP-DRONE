...
| `disconnect` | implicit CLI session close after command exit | yes | no | yes | yes | yes | `PASS` by repeated open/close across the serial matrix |
...
| `motor-test` | `... motor-test m1 0.05` | yes | yes | yes | yes | yes | `PASS` on serial at low duty (`m1..m4`, on/off) |
| `axis-test` | `... axis-test roll 0.05` | yes | yes | yes | yes | yes | `PASS` on serial command path for `roll/pitch/yaw` |
| `rate-test` | `... rate-test roll 20` | yes | yes | yes | yes | yes | `PASS` on serial command path for `roll/pitch/yaw` |
...
- There is no standalone `disconnect` subcommand today. CLI disconnect is implicit in `DeviceSession.close()` when the process exits.
- Serial validation was completed on `COM4` on `2026-04-03`.
- `DIRECT` mode now requests `attitude + quaternion + gyro/acc`, so `gyro_x/y/z`, `calib gyro`, and the rate-test command path are available without switching to `RAW`.
- `reboot` is considered verified only after `connect` succeeded again on the same `COM4` port.
- UDP validation is still deferred. The device-side UDP path is not implemented yet in the current firmware task graph, so this does not block serial CLI acceptance.
