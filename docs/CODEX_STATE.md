...
- WiFi debug transport: firmware default `wifi_mode=sta`. If `sta_ssid` is empty at boot, firmware falls back to SoftAP `192.168.4.1:2391`. When `sta_ssid` and `sta_password` are configured, the drone joins the router/hotspot in STA mode; STA success logs the acquired IP, STA failure falls back to SoftAP.
