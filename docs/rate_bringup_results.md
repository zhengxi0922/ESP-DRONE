...
- On `2026-04-01`, this workstation reported `no ports found` for `python -m serial.tools.list_ports -v`. No real bench measurement was captured in this session, so the table below remains a blocked execution record rather than a completed hardware result set.
...
| Z-axis physical rotation -> project yaw-rate | Rotate nose right / nose left in yaw | `rate-test yaw 30` and `rate-test yaw -30` | `gyro_z` negative for `+yaw`, positive for `-yaw` | `rate_setpoint_yaw` | `rate_pid_p_yaw`, `rate_pid_i_yaw`, `rate_pid_d_yaw`, `pid_out_yaw` | `+yaw`: `M1/M3` above `M2/M4`; `-yaw`: `M2/M4` above `M1/M3` | Not executed in this session because no USB CDC port was available | Blocked |
