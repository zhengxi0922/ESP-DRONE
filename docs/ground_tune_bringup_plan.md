...
- Roll/pitch attitude outer loop is disabled in rate-only ground tune by default. It is forced on only for explicit `ATTITUDE_VERIFY`, `LOW_RISK_LIFTOFF`, and `UDP_MANUAL` submodes.
- Yaw remains rate inner-loop only. No yaw heading hold is enabled.
- Existing `rate-test`, `axis-bench`, `rate-bench`, `attitude-capture-ref`, `attitude-test`, and `watch-attitude` remain separate. UDP manual no longer uses the hang-attitude reference; roll/pitch are unified through the flat-ground reference outer loop and the tuned rate PID.
...
4. Default Ground Tune outputs `rate_sp_roll = rate_sp_pitch = rate_sp_yaw = 0`. If `ground_tune_enable_attitude_outer=true` or the submode forces the outer loop, roll/pitch error is computed from the flat-ground reference. If `ground_tune_use_kalman_attitude=true`, the captured Kalman deltas are used as the roll/pitch measurement for the outer loop.
...
- `ground_att_kp_roll = 0.8`
- `ground_att_kp_pitch = 0.8`
- `ground_att_rate_limit_roll = 4 deg/s`
- `ground_att_rate_limit_pitch = 4 deg/s`
- `ground_att_target_limit_deg = 2 deg`
- `ground_att_error_deadband_deg = 0.2 deg`
- `ground_att_trip_deg = 12 deg`
- `ground_test_max_extra_duty = 0.03`
- `ground_test_motor_balance_limit = 0.06`
- `ground_test_auto_disarm_ms = 15000`
- `ground_test_ramp_duty_per_s = 0.15`
- `gyro_lpf_hz = 40 Hz`
- `accel_lpf_hz = 20 Hz`
- `rate_lpf_hz = 30 Hz`
- `kalman_enable = true`
...
- `liftoff_verify_base_duty = 0.10`
- `liftoff_verify_max_extra_duty = 0.04`
- `liftoff_verify_auto_disarm_ms = 2500`
- `liftoff_verify_ramp_duty_per_s = 0.10`
- `liftoff_verify_att_trip_deg = 8 deg`
