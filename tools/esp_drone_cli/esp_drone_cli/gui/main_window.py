...
EXTRA_TRANSLATIONS = {
    "zh": {
        ...
        "msg.sta_host_required": "STA 模式下需要填写无人机 IP。请查看串口日志中的 sta connected ip=...，将该 IP 填入 UDP Host。",
        ...
        "wifi.auto_write_before_save": "检测到配置已变更，自动执行写入并校验...",
        "wifi.save_blocked_write_failed": "写入/校验未通过，已阻止保存和重启。请检查串口连接和参数值后重试。",
    ...
    "en": {
        ...
        "msg.sta_host_required": "STA mode requires the drone IP. Check serial logs for 'sta connected ip=...' and enter that IP in UDP Host.",
        ...
        "wifi.auto_write_before_save": "Config changed detected, auto-writing and verifying...",
        "wifi.save_blocked_write_failed": "Write/verify failed; save and reboot blocked. Check serial connection and parameter values, then retry.",
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self._wifi_config_dirty = False
        self._last_wifi_write_ok = False
        self._loading_wifi_settings = False
    ...
    def _build_connection_group(self) -> QGroupBox:
        ...
        self._apply_compact_field(self.link_type_combo, 112)
        ...
        serial_port_row_layout.addWidget(self.serial_port_combo)
        serial_port_row_layout.addWidget(self.refresh_ports_button)
        serial_port_row_layout.addStretch(1)
        self._apply_compact_button(self.refresh_ports_button, 72)
        serial_form.setColumnStretch(0, 0)
        serial_form.setColumnStretch(1, 0)
        serial_form.setColumnStretch(2, 1)
        serial_form.addWidget(self.serial_port_label, 0, 0)
        serial_form.addWidget(self.serial_port_row, 0, 1, 1, 2)
        serial_form.addWidget(self.baud_label, 1, 0)
        serial_form.addWidget(self.baudrate_spin, 1, 1, Qt.AlignLeft)
        ...
        self._apply_compact_button(self.connect_button, 96)
        self._apply_compact_button(self.disconnect_button, 96)
        ...
        link_type_row.setSpacing(6)
        link_type_row.addWidget(self.link_type_label)
        link_type_row.addWidget(self.link_type_combo)
        link_type_row.addWidget(self.connection_status_chip)
        link_type_row.addStretch(1)

        connection_button_row = QHBoxLayout()
        connection_button_row.setContentsMargins(0, 0, 0, 0)
        connection_button_row.setSpacing(6)
        connection_button_row.addWidget(self.connect_button)
        connection_button_row.addWidget(self.disconnect_button)
        connection_button_row.addStretch(1)

        layout.addLayout(link_type_row, 0, 0, 1, 4)
        layout.addWidget(self.transport_stack, 1, 0, 1, 4)
        layout.addLayout(connection_button_row, 2, 0, 1, 4)
    ...
    def _build_safety_group(self) -> QGroupBox:
        ...
        self._apply_compact_button(self.arm_button, 112)
        self._apply_compact_button(self.disarm_button, 112)
        self._apply_compact_button(self.kill_button, 112)
        self._apply_compact_button(self.reboot_button, 92)

        layout.addWidget(self.arm_button, 0, 0, Qt.AlignLeft)
        layout.addWidget(self.disarm_button, 0, 1, Qt.AlignLeft)
        layout.addWidget(self.kill_button, 1, 0, Qt.AlignLeft)
        layout.addWidget(self.reboot_button, 1, 1, Qt.AlignLeft)
        return group
    ...
    def _build_debug_group(self) -> QGroupBox:
        ...
        motor_layout.setColumnStretch(0, 0)
        motor_layout.setColumnStretch(1, 0)
        motor_layout.setColumnStretch(2, 0)
        motor_layout.setColumnStretch(3, 0)
        motor_layout.setColumnStretch(4, 1)
        ...
        self._apply_compact_field(self.motor_duty_spin, 112)
        motor_layout.addWidget(self.motor_label, 0, 0)
        motor_layout.addWidget(self.motor_combo, 0, 1, Qt.AlignLeft)
        motor_layout.addWidget(self.duty_label, 0, 2)
        motor_layout.addWidget(self.motor_duty_spin, 0, 3, Qt.AlignLeft)
        motor_layout.addWidget(self.motor_start_button, 1, 1, Qt.AlignLeft)
        motor_layout.addWidget(self.motor_stop_button, 1, 2, Qt.AlignLeft)
        self.motor_group = motor_box
        ...
        rate_layout.setColumnStretch(0, 0)
        rate_layout.setColumnStretch(1, 0)
        rate_layout.setColumnStretch(2, 0)
        rate_layout.setColumnStretch(3, 0)
        rate_layout.setColumnStretch(4, 1)
        ...
        self._apply_compact_field(self.rate_value_spin, 112)
        rate_layout.addWidget(self.rate_axis_label, 0, 0)
        rate_layout.addWidget(self.rate_axis_combo, 0, 1, Qt.AlignLeft)
        rate_layout.addWidget(self.rate_dps_label, 0, 2)
        rate_layout.addWidget(self.rate_value_spin, 0, 3, Qt.AlignLeft)
        rate_layout.addWidget(self.rate_start_button, 1, 1, Qt.AlignLeft)
        rate_layout.addWidget(self.rate_stop_button, 1, 2, Qt.AlignLeft)
        self.rate_group = rate_box
        ...
        ground_layout.addWidget(self.ground_record_5_button, 2, 0)
        ground_layout.addWidget(self.ground_record_10_button, 2, 1)
        ground_layout.addWidget(self.ground_bench_roll_button, 3, 0)
        ...
        ground_params = [
            ...
            ("ground_att_kp_roll", "ground_att_kp_roll", 0.0, 10.0, 2, 0.1, 0.8),
            ("ground_att_kp_pitch", "ground_att_kp_pitch", 0.0, 10.0, 2, 0.1, 0.8),
            ("ground_att_rate_limit_roll", "ground_att_rate_limit_roll", 0.0, 60.0, 1, 1.0, 4.0),
            ("ground_att_rate_limit_pitch", "ground_att_rate_limit_pitch", 0.0, 60.0, 1, 1.0, 4.0),
            ("ground_att_target_limit_deg", "ground_target_limit", 0.5, 10.0, 1, 0.5, 2.0),
            ("ground_att_error_deadband_deg", "ground_deadband", 0.0, 5.0, 1, 0.1, 0.2),
            ("ground_att_trip_deg", "ground_trip", 5.0, 30.0, 1, 1.0, 12.0),
            ("ground_test_base_duty", "ground_base_duty", 0.0, 1.0, 3, 0.01, 0.08),
            ("ground_test_max_extra_duty", "ground_max_extra_duty", 0.0, 0.20, 3, 0.01, 0.03),
            ("ground_test_motor_balance_limit", "ground_motor_balance_limit", 0.0, 0.30, 3, 0.01, 0.06),
            ("ground_test_ramp_duty_per_s", "ground_ramp_duty_per_s", 0.01, 2.0, 2, 0.05, 0.15),
    ...
    def _wire_signals(self) -> None:
        ...
        self.wifi_mode_combo.currentIndexChanged.connect(lambda *_args: (self._save_settings(), self._mark_wifi_dirty()))
        self.wifi_ssid_edit.textChanged.connect(lambda *_args: (self._save_settings(), self._mark_wifi_dirty()))
        self.wifi_password_edit.textChanged.connect(lambda *_args: self._mark_wifi_dirty())
        self.wifi_static_ip_edit.textChanged.connect(lambda *_args: (self._save_settings(), self._mark_wifi_dirty()))
        self.wifi_gateway_edit.textChanged.connect(lambda *_args: (self._save_settings(), self._mark_wifi_dirty()))
        self.wifi_netmask_edit.textChanged.connect(lambda *_args: (self._save_settings(), self._mark_wifi_dirty()))
    ...
    def _write_wifi_config(self) -> None:
        ...
        self._wifi_write_in_progress = False
        self._wifi_config_dirty = False
        self._last_wifi_write_ok = True

    def _mark_wifi_dirty(self) -> None:
        if self._loading_wifi_settings:
            return
        self._wifi_config_dirty = True
        self._last_wifi_write_ok = False
    ...
    def _save_and_reboot_wifi(self) -> None:
        ...
        if self._wifi_config_dirty or not self._last_wifi_write_ok:
            self._append_log(self._t("wifi.auto_write_before_save"))
            self._write_wifi_config()
            if self._wifi_config_dirty or not self._last_wifi_write_ok:
                self._append_log(self._t("wifi.save_blocked_write_failed"))
                return
    ...
    def _connect_requested(self) -> None:
        ...
        if not host:
            mode = self._udp_mode_value()
            if mode == "sta":
                self._show_connect_failure(self._t("msg.sta_host_required"))
            else:
    ...
    def _load_settings(self) -> None:
        ...
        self._loading_wifi_settings = True
        try:
